...
const TimeBasedBoardingAnalysis = () => {
  ...
  // Sort data by hour
  const sortedData = [...processedData].sort((a, b) => a.hourValue - b.hourValue);
  
  // Determine color intensity for heatmap
  const maxCount = Math.max(...sortedData.map(item => item.count));
  
  if (isLoading) return <div className="flex justify-center items-center h-64">Loading boarding data...</div>;
  
  if (error) return <div className="text-red-500 p-4">Error loading boarding data</div>;
  
  if (!data.length) return <div className="p-4">No boarding data available</div>;
    ...
    <div className="w-full bg-white rounded-lg shadow p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          <button 
            className={`px-4 py-2 rounded-[20px] ${viewType === 'line' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
            onClick={() => setViewType('line')}
          >
            Line Graph
          </button>
          <button 
            className={`px-4 py-2 rounded-[20px] ${viewType === 'heatmap' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
      ...
      {viewType === 'line' ? (
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={sortedData} margin={{ top: 5, right: 30, left: 20, bottom: 10}}>
              ...
              />
              <YAxis 
                label={{ value: 'Boarding Count', angle: -90, position: 'insideLeft' }} 
              />
              <Tooltip 
